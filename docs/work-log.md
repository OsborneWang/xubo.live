...
## 2026-05-30 23:50 - Hero 红框留白压缩

### 背景

用户通过截图标注 Hero 区域中状态栏下方、左侧文案上方以及右侧 profile 卡片上方仍存在较大留白，希望整体更紧凑。

### 目标

- 压缩 Hero 顶部和左右两栏上方的空白区域。
- 保持现有双栏结构和整体视觉风格。
- 最小化 CSS 改动。

### 执行内容

- 将 `.hero-grid` 的 `align-items` 从 `end` 改为 `start`，避免左侧文案为了与右侧高卡片底部对齐而被压到更低位置。
- 将 `.hero-grid` 的顶部间距从 `clamp(36px, 8vh, 88px)` 收紧为 `clamp(18px, 3vh, 32px)`。
- 将 `.hero-copy` 顶部内边距从 `24px` 收紧为 `8px`。
- 同步调整 860px 以下断点的 `.hero-copy` 顶部内边距，避免移动端重新出现过大空白。
- 执行 Astro 构建验证。
...
- 已执行 `npm run build`。
- 构建成功，Astro 生成 2 个页面：`/index.html` 和 `/404.html`。
- 本地 dev server 若仍运行，可直接刷新 `http://127.0.0.1:4321/` 预览。

### 关键决策

- 留白根因是双栏底部对齐导致左侧内容下沉，因此优先改为顶部对齐。
- 不调整文案内容与卡片结构，仅收紧布局参数。

### 后续建议

- 如果用户希望首屏更像“仪表盘密集布局”，可进一步减小标题字号或降低 profile 卡片内容行高。
